...
func (s EnhanceService) Review(ctx context.Context, root string) (CurrentState, ReviewPlan, error) {
	current, plan, _, err := s.review(ctx, root)
	return current, plan, err
}

func (s EnhanceService) review(ctx context.Context, root string) (CurrentState, ReviewPlan, Evidence, error) {
	current, err := LoadCurrentState(root)
	if err != nil {
		return CurrentState{}, ReviewPlan{}, Evidence{}, err
	}
	if current.Project.GeneratedBy != "promptgrinder discover" {
		return CurrentState{}, ReviewPlan{}, Evidence{}, fmt.Errorf("missing discovery configuration: .promptgrinder/project.yaml was not generated by promptgrinder discover")
	}
	if s.Advisor == nil {
		return CurrentState{}, ReviewPlan{}, Evidence{}, fmt.Errorf("role advisor runtime is not configured")
	}
	...
	for _, collector := range collectors {
		...
		if e != nil {
			return CurrentState{}, ReviewPlan{}, Evidence{}, e
		}
	...
	if err != nil {
		return current, ReviewPlan{}, evidence, err
	}
	...
	return current, validated, evidence, err
}
