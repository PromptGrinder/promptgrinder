...
func TestSharedProgressRendererPlainLifecycle(t *testing.T) {
	...
	progress.Scope, progress.Engine, progress.Model = "slice-policy", "codex", "gpt-5.6-sol"
	...
	for _, want := range []string{
		...
		"✓ [2/3] 02B-task.md|slice-policy|codex/gpt-5.6-sol|4m 39s",
	} {
...
func TestSharedProgressRendererPlainFailure(t *testing.T) {
	...
	renderer.Update(pgruntime.SharedRunProgress{
		...
		Duration: 12*time.Second + 600*time.Millisecond, Engine: "codex",
	})

	if got := out.String(); !strings.Contains(got, "✗ [1/2] 01-task.md|unscoped|codex/default|13s") {
